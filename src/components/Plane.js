...
const Plane = (props) => {
  ...
  React.useEffect(() => {
    if (grassColorTexture) {
      grassColorTexture.repeat.set(64, 64);
  ...
  React.useEffect(() => {
    if (grassAmbientOcclusionTexture) {
      grassAmbientOcclusionTexture.repeat.set(64, 64);
      grassAmbientOcclusionTexture.wrapS = grassAmbientOcclusionTexture.wrapT =
  ...
  React.useEffect(() => {
    if (grassNormalTexture) {
      grassNormalTexture.wrapS = grassNormalTexture.wrapT =
        THREE.RepeatWrapping;
      grassNormalTexture.repeat.set(64, 64);
    }
  }, [grassNormalTexture]);

  React.useEffect(() => {
    if (grassRoughnessTexture) {
      grassRoughnessTexture.repeat.set(64, 64);
      grassRoughnessTexture.wrapS = grassRoughnessTexture.wrapT =
        THREE.RepeatWrapping;
    }
  }, [grassRoughnessTexture]);

  return (
    <mesh {...props} rotation={[-Math.PI * 0.5, 0, 0]} receiveShadow>
      <planeGeometry ref={geomRef} args={[100, 100]} />
